        ...
        if (sub8x8_inter) {
            ...
            for (int32_t y = 0; y < b8_h; y += b4_h) {
                ...
                for (int32_t x = 0; x < b8_w; x += b4_w) {
                    ...
                    EbPictureBufferDesc_t  *ref_pic = this_mbmi->ref_frame[0] ==
                        LAST_FRAME || this_mbmi->ref_frame[0] == LAST2_FRAME || this_mbmi->ref_frame[0] == LAST3_FRAME || this_mbmi->ref_frame[0] == GOLDEN_FRAME    ?
                        ((EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_0][ref_idx]->object_ptr)->referencePicture : 
                        ((EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_1][ref_idx]->object_ptr)->referencePicture;
#else
        ...
        if (sub8x8_inter) {
            ...
            for (int32_t y = 0; y < b8_h; y += b4_h) {
                ...
                for (int32_t x = 0; x < b8_w; x += b4_w) {
                    ...
                    EbPictureBufferDesc_t  *ref_pic = this_mbmi->ref_frame[0] ==
                        LAST_FRAME || this_mbmi->ref_frame[0] == LAST2_FRAME || this_mbmi->ref_frame[0] == LAST3_FRAME || this_mbmi->ref_frame[0] == GOLDEN_FRAME ?
                        ((EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_0][ref_idx]->object_ptr)->referencePicture16bit :
                        ((EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_1][ref_idx]->object_ptr)->referencePicture16bit;
#else
                    EbPictureBufferDesc_t                  *ref_pic = this_mbmi->ref_frame[0] == LAST_FRAME ? ref_pic_list0 : ref_pic_list1;
#endif
                    src_ptr = (uint16_t*)ref_pic->bufferCb + (ref_pic->origin_x + ((pu_origin_x >> 3) << 3)) / 2 + (ref_pic->origin_y + ((pu_origin_y >> 3) << 3)) / 2 * ref_pic->strideCb;
    ...
    EbAsm                                   asm_type)
{
    ...
    if (is16bit) {
        if (ref_idx_l0 >=0)
            ...
            ref_pic_list0 = (EbPictureBufferDesc_t*)EB_NULL;
        if (ref_idx_l1 >= 0)
            ref_pic_list1 = ((EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_1][ref_idx_l1]->object_ptr)->referencePicture16bit;
        else
            ref_pic_list1 = (EbPictureBufferDesc_t*)EB_NULL;
    }
    else {
        if (ref_idx_l0 >= 0)
             ref_pic_list0 =  ((EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_0][ref_idx_l0]->object_ptr)->referencePicture;
        else
            ref_pic_list0 = (EbPictureBufferDesc_t*)EB_NULL;
        if (ref_idx_l1 >= 0)
            ref_pic_list1 = ((EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_1][ref_idx_l1]->object_ptr)->referencePicture;
        else
            ref_pic_list1 = (EbPictureBufferDesc_t*)EB_NULL;
    }
#endif
