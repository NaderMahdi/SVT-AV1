...
#else
    if (cu_origin_y != 0) {//dcSignLevelCoeffNeighborArray->topArray[dcSignLevelCoeffTopNeighborIndex] != INVALID_NEIGHBOR_DATA){ 
#endif
        do {
...
#else
    if (cu_origin_x != 0) {// dcSignLevelCoeffNeighborArray->leftArray[dcSignLevelCoeffLeftNeighborIndex] != INVALID_NEIGHBOR_DATA){ 
#endif
        k = 0;
        do {
...
static void write_ref_frames(
    FRAME_CONTEXT               *frame_context,
    PictureParentControlSet_t   *pcs_ptr,
    const MacroBlockD           *xd,
    aom_writer                  *w) {
...
static void write_film_grain_params(PictureParentControlSet_t *pcsPtr,
    ...
    if (pcsPtr->av1FrameType == INTER_FRAME) {
#if MRP_MD
        EbReferenceObject_t* refObj0 = (EbReferenceObject_t*)pcsPtr->childPcs->ref_pic_ptr_array[REF_LIST_0][0]->object_ptr;
#else
        ...
        {
#if MRP_MD
            EbReferenceObject_t* refObj1 = (EbReferenceObject_t*)pcsPtr->childPcs->ref_pic_ptr_array[REF_LIST_1][0]->object_ptr;
#else
    ...
    EbPictureBufferDesc_t   *coeffPtr)
{
    ...
    cu_ptr->av1xd->up_available     = (mi_row > tbPtr->tile_info.mi_row_start);
    cu_ptr->av1xd->left_available   = (mi_col > tbPtr->tile_info.mi_col_start);
